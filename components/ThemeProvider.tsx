...
export function ThemeToggle({ className }: { className?: string }) {
  ...
  if (!mounted) {
    return <div className={`p-2 w-[34px] h-[34px] ${className ?? ""}`} />;
  }

  return (
    <button
      onClick={toggle}
      className={`p-2 rounded-[var(--radius-md)] transition-colors ${className ?? ""}`}
      style={{ color: theme === "dark" ? "var(--color-accent)" : "var(--color-text-secondary)" }}
